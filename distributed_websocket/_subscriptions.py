...
def _is_subscription_message(message: Message) -> bool:
...
def _check_subscription_message(message: Message) -> bool:
    return _is_subscription_message(message) and _is_valid_subscription(message.topic)


def subscribe(connection: Connection, message: Message) -> NoReturn:
    assert _check_subscription_message(message), f'Message {message} is not valid for sub/unsub'
